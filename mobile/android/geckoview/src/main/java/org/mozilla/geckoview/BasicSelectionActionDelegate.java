...
public class BasicSelectionActionDelegate
    ...
    for (int i = 0; i < allActions.length; i++) {
      ...
      if (ACTION_PROCESS_TEXT.equals(actionId)) {
        if (mExternalActionsEnabled && !mSelection.text.isEmpty()) {
          menu.addIntentOptions(menuId, menuId, menuId, mActivity.getComponentName(),
  ...
  public void onGetContentRect(
    ...
    Log.d(LOGTAG, "view local visible rect: " + viewRect);

    RectF clientBoundRect = new RectF();
    mSession.getClientBounds(clientBoundRect);
    Log.d(LOGTAG, "clientBound rect: " + clientBoundRect);

    Log.d(LOGTAG, "before outRect: " + outRect);
    mSession.getClientToScreenMatrix(mTempMatrix);
    Log.d(LOGTAG, "mSelection.clientRect: " + mSelection.clientRect);
    mTempMatrix.mapRect(mTempRect, mSelection.clientRect);
    Log.d(LOGTAG, "mTempRect: " + mTempRect);
    mTempRect.roundOut(outRect);
    Log.d(LOGTAG, "after outRect: " + outRect);
  }

  @TargetApi(Build.VERSION_CODES.M)
  @Override
  public void onShowActionRequest(final GeckoSession session, final Selection selection,
      final String[] actions, final GeckoResponse<String> response) {
    ThreadUtils.assertOnUiThread();
    mSession = session;
