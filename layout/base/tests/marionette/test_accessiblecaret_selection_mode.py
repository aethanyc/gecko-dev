...
class AccessibleCaretSelectionModeTestCase(MarionetteTestCase):
    ...
    def test_caret_not_show_after_key_scroll_down_and_up(self):
        self.open_test_html(self._key_scroll_html)
        html = self.marionette.find_element(By.ID, "html")
        sel = SelectionManager(html)

        # Select "BBBBB" to get the y position of the second caret. This is the
        ...
        self.actions.key_chain.send_keys(Keys.ARROW_DOWN)\
                              .pause(1000).send_keys(Keys.ARROW_UP).perform()
        self.assertEqual("AAAAA", sel.selected_content)

        # Step 3: Drag the second caret down. The carets shouldn't show up.
        # Otherwise, we can drag the second caret to extend the selection to
        # "BBBBB".
