...
struct TableReflowInput final {
  ...
  // The table's available size (in mReflowInput's writing mode)
...
void nsTableFrame::Reflow(nsPresContext* aPresContext,
  ...
  if (IsSubtreeDirty() || aReflowInput.ShouldReflowAllKids() ||
    ...
    if (needToInitiateSpecialReflow && aStatus.IsComplete()) {
      ...
      CalcDesiredBSize(aReflowInput, borderPadding, aDesiredSize);
...
void nsTableFrame::ReflowTable(ReflowOutput& aDesiredSize,
  ...
  printf(
      "table %s: aBorderPadding %s, orig availSize %s, availSize %s, mICoord "
      "%d, mBCoord "
      "%d\n",
      ListTag().get(), ToString(aBorderPadding).c_str(),
...
void nsTableFrame::CalcDesiredBSize(const ReflowInput& aReflowInput,
  ...
  if (!GetPrevInFlow()) {
    ...
    if (bSize > desiredBSize) {
      ...
      DistributeBSizeToRows(aReflowInput, aBorderPadding, bSize - desiredBSize);
...
void nsTableFrame::DistributeBSizeToRows(const ReflowInput& aReflowInput,
  ...
  nscoord bOriginRG = aBorderPadding.BStart(wm) + GetRowSpacing(0);
  ...
  bOriginRG = aBorderPadding.BStart(wm) + GetRowSpacing(-1);
  bEndRG = bOriginRG;
  for (rgIdx = 0; rgIdx < rowGroups.Length(); rgIdx++) {
    nsTableRowGroupFrame* rgFrame = rowGroups[rgIdx];
    nscoord amountUsedByRG = 0;
