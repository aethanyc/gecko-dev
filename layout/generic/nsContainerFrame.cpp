...
void nsContainerFrame::NormalizeChildLists() {
  ...
  // This method won't work if the following bits do not have the same values
  // in the flex container group and the grid container group.
  static_assert(NS_STATE_FLEX_DID_PUSH_ITEMS == NS_STATE_GRID_DID_PUSH_ITEMS);
  static_assert(NS_STATE_FLEX_HAS_CHILD_NIFS == NS_STATE_GRID_HAS_CHILD_NIFS);

  constexpr nsFrameState didPushItemsBit = NS_STATE_FLEX_DID_PUSH_ITEMS;
  constexpr nsFrameState hasChildNifBit = NS_STATE_FLEX_HAS_CHILD_NIFS;
    ...
    while (nif) {
      ...
      if (!nif->HasAnyStateBits(didPushItemsBit)) {
        MOZ_ASSERT(!nifNeedPushedItem || mDidPushItemsBitMayLie,
                   "NS_STATE_FLEX_DID_PUSH_ITEMS or "
...
void nsContainerFrame::SanityCheckChildListsBeforeReflow() const {
  ...
  for (const nsIFrame* f = this; f; f = f->GetNextInFlow()) {
    MOZ_ASSERT(!f->HasAnyStateBits(NS_STATE_FLEX_DID_PUSH_ITEMS |
                                   NS_STATE_GRID_DID_PUSH_ITEMS),
               "At start of reflow, we should've pulled items back from all "
               "NIFs and cleared NS_STATE_FLEX_DID_PUSH_ITEMS or "
               "NS_STATE_GRID_DID_PUSH_ITEMS in the process");
