...
LogicalSize nsGridContainerFrame::GridReflowInput::PercentageBasisFor(
  ...
  if (aAxis == LogicalAxis::Inline || !mCols.mCanResolveLineRangeSize) {
    if (StaticPrefs::layout_css_grid_multi_pass_track_sizing_enabled() &&
      ...
      // When calculating column sizes with definite row sizes, compute the row
      // size for the grid area placed by aGridItem.
...
nscoord nsGridContainerFrame::ComputeIntrinsicISize(
  ...
  auto GetContentISize = [&]() {
    return !IsColSubgrid()
               ? gridRI.mCols.SumOfGridTracksAndGaps()
               : gridRI.mCols.GridLineEdge(gridRI.mCols.mSizes.Length(),
                                           GridLineSide::BeforeGridGap);
  };

  if (StaticPrefs::layout_css_grid_multi_pass_track_sizing_enabled()) {
    const nscoord contentISize = GetContentISize();
    ...
    // row sizes. This ensures that those grid items also with aspect-ratios can
    // transfer the resolved block-size to the inline axis, contributing to the
    // grid container's intrinsic inline-size.
    ...
    gridRI.CalculateTrackSizesForAxis(LogicalAxis::Inline, grid, contentISize,
                                      constraint);
  }

  return GetContentISize();
}
