...
nscoord nsGridContainerFrame::ComputeContentBoxBSize(
    ...
    nscoord aIntrinsicContentBoxBSize,
    const Maybe<nscoord>& aContainIntrinsicBSize) const {
  if (aContainIntrinsicBSize) {
  ...
  if (!IsRowSubgrid()) {
    if (StaticPrefs::layout_css_grid_multi_pass_track_sizing_enabled() &&
      ...
      // Use the intrinsic content-box block-size that was determined after we
      // resolved the row sizes the first time in
...
void nsGridContainerFrame::Reflow(nsPresContext* aPresContext,
  ...
  if (MOZ_LIKELY(!prevInFlow)) {
    ...
    const nscoord intrinsicContentBoxBSize = ComputeIntrinsicContentBoxBSize(
    ...
    contentBSize = ComputeContentBoxBSize(
        gridRI, computedBSize, intrinsicContentBoxBSize, containIntrinsicBSize);
  } else {
